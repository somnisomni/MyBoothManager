  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<unknown> {
    ...
    if(contextClass === AppController && contextHandler === AppController.prototype.teapot) {
      return next.handle().pipe(tap({
        finalize: () => {
          console.log(logFn("🍵", now, chalk`health check from {bold ${contextHttp.ip}}`));
          console.log();
        },
      }));
    }
    ...
    return next.handle().pipe(tap({
      next: () => {
        console.log(logFn("✅", now, chalk`request from {bold ${contextHttp.ip}}`));
      ...
      error: (error) => {
        console.error(logFn("❌", now, chalk`request from {bold ${contextHttp.ip}}`));
