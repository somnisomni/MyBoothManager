<template>
  <VContainer class="mt-4 px-0 px-sm-2 py-2 pa-md-6 d-flex flex-column">
    <div v-if="Object.keys(orderHistory).length > 0">
      ...
      <VSlideGroup v-model="currentSelectedDay"
                   class="justify-center px-2 mb-6"
                   showArrows
                   centerActive
                   mandatory>
      ...
      <Chart class="bg-background"
             type="line"
             :options="CHART_ANALYTICS_OPTIONS"
             :data="chartOrderHistoryDataOfSelectedDay" />
...
class AnalyticsPage extends Vue {
    ...
    plugins: {
      ...
      legend: {
        onClick(e, legendItem) {
          ...
          this.chart.scales[this.chart.data.datasets[index].stack as string].options.display = !this.chart.data.datasets[index].hidden;
          this.chart.update();
        },
      },
    },
  ...
  get orderHistoryFilteredArray(): IGoodsOrder[] {
  ...
  get orderHistoryDays(): OrderedDateonlySet {
    ...
    this.orderHistoryFilteredArray.forEach((item) => {
      if(item.createdAt) {
        set.add(Dateonly.fromDate(item.createdAt));
      }
    });
  ...
  get orderHistoryByDay(): Record<number, IGoodsOrder[]> {
    const merged: Record<number, IGoodsOrder[]> = { };
    ...
    this.orderHistoryFilteredArray.forEach((item) => {
      ...
      if(!merged[targetTimestamp]) {
        merged[targetTimestamp] = [];
      }
  ...
  get orderHistoryOfCurrentSelectedDay(): IGoodsOrder[] {
    if(this.currentSelectedDay === "all") {
      return this.orderHistoryFilteredArray;
    } else {
      return this.orderHistoryByDay[this.currentSelectedDay.getTimestamp()] ?? [];
    }
  }
  ...
  get orderHistoryInfoMergedByMinute(): Record<number, { quantity: number; price: number }> {
    ...
    this.orderHistoryFilteredArray.forEach((item) => {
      ...
      if(!merged[targetTimestamp]) {
        merged[targetTimestamp] = { quantity: 0, price: 0 };
      }
    ...
    return {
      datasets: [ {
        ...
        data: Object.entries(this.orderHistoryInfoMergedByMinute).map((item) => {
          ...
          if(this.currentSelectedDay !== "all" && !Dateonly.fromDate(date).equals(this.currentSelectedDay)) {
            return;
          }
        ...
        }).filter(item => typeof item !== "undefined") as Point[],
