  ...
  <VContainer class="pa-0 pa-sm-2 pa-md-6">
    <VLayout class="pa-2 d-flex align-center align-sm-end flex-column flex-sm-row">
      <div>
        ...
        <VLayout class="mt-2 d-flex align-center align-sm-center flex-column flex-sm-row">
          ...
          <VDivider class="my-1 my-sm-0 mx-2 w-100 w-sm-auto"
                    :vertical="smAndUp" />
          <VSheet class="mx-1"
                  variant="outlined"
                  @click.prevent>
    ...
    <VLayout class="pa-2">
      <VBtn size="large"
            ...
            @click="filterSettingDialogShown = true">
        <span>필터 설정</span>
      </VBtn>

      <div>
        <!-- TODO: v-if filterSettings is not default -->
    ...
    <h2 v-else-if="!dataLoading && Object.keys(boothGoodsOrders).length <= 0"
        class="text-center">
      등록된 판매 기록이 없습니다.
    </h2>
...
<script lang="ts">
import type { IGoodsOrderFilterResult, IGoodsOrderFilterSetting } from "@/components/goods/GoodsOrderListView.vue";
...
class GoodsOrdersListPage extends Vue {
  ...
  @Hook
  beforeRouteEnter(to: RouteRecordRaw, from: RouteRecordRaw, next: (callback?: (vm?: GoodsOrdersListPage) => void) => void): void {
    next((vm) => {
      if(vm) {
  ...
  @Hook
  beforeRouteLeave(to: RouteRecordRaw): void {
    to.meta = { previousScrollOffset: window.scrollY };
  }

  get boothGoodsOrders(): Record<number, IGoodsOrder> {
  ...
  setOrderFilter(filterSetting: IGoodsOrderFilterSetting): void {
